#' @title Fitting Semi-logarithmic regression and Plotting
#' @param stan dataFrame for fitting semi-logarithmic regression
...
#' @description Fitting data with semi-logarithmic regression.
...
semilog <- function(stan, res, stan.x = NULL, stan.y = NULL, ppt.fw = 5,
  ...
  fit <- lm(formula = stan[, stan.y] ~ I(log(stan[, stan.x])),data = stan)
  cat("Semi-logarithmic regression")
  print(fit)
  label <- data.frame(
    formula = sprintf("italic(y) == %.2f %+.2f * italic(log(x))",
                      round(coef(fit)[1], 2), round(coef(fit)[2], 2)),
  ...
  p <- ggplot(data = stan, aes(x = stan[, stan.x], y = stan[, stan.y])) +
    geom_point() + geom_smooth(method = lm, formula = y ~ I(log(x)) ,se = F) +
    xlab(label = stan.x) + ylab(label = stan.y) +
    labs(title = "Semi-logarithmic regression") +
  ...
  print(p)
  if (isTRUE(toppt)) {
    topptx(figure = p, filename = "Semi-logarithmic regression.pptx",
  ...
  semilogFit <- list(predict = predict_fit, plot = p)
  return(semilogFit)
}
