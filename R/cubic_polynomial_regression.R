#' @title Fitting Cubic polynomial regression and Plotting
#' @param stan dataFrame for fitting cubic polynomial regression
...
#' @description Fitting data with cubic polynomial regression.
...
cubic <- function(stan, res, stan.x = NULL, stan.y = NULL, ppt.fw = 5,
  ...
  fit <- lm(formula = stan[, stan.y] ~ stan[, stan.x] + I(stan[, stan.x]^2) +
              I(stan[, stan.x]^3), data = stan)
  cat("Cubic polynomial regression")
  print(fit)
  label <- data.frame(
    formula = sprintf("italic(y) == %.2f %+.2f * italic(x) %+.2f * italic(x^2)
                      %+.2f * italic(x^3)",
                      round(coef(fit)[1], 2), round(coef(fit)[2], 2),
                      round(coef(fit)[3], 2), round(coef(fit)[4], 2)),
  ...
  p <- ggplot(data = stan, aes(x = stan[, stan.x], y = stan[, stan.y])) +
    geom_point() + xlab(label = stan.x) + ylab(label = stan.y) +
  ...
  print(p)
  if (isTRUE(toppt)) {
    topptx(figure = p, filename = "Cubic polynomial regression.pptx",
  ...
  cubicFit <- list(predict = predict_fit, plot = p)
  return(cubicFit)
}
